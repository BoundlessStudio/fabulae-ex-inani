...
This is the original upstream Mapgen4 README kept for attribution and historical context.
Some commands and paths below refer to the upstream browser project, not the current Node CLI.
Use the repository root =README.md= for current workflow instructions.
