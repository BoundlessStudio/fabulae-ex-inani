...
Open `http://127.0.0.1:8787/index.html`. The viewer has a starter archive embedded in `index.html`, but full index search and rich on-demand pages use generated `indexes/`, `records/`, and `mentions/` JSON chunks. Serving the folder over HTTP avoids browser `file://` restrictions for those chunk requests.
...
This uses a small 128px render target, enables phase timing checkpoints, and writes `output/stress-probes/probe-500-final.png`, `output/stress-probes/probe-500-final.json`, and 100-year checkpoint profiles in `output/stress-probes/probe-500-profiles/` as `year-100.json`, `year-200.json`, `year-300.json`, `year-400.json`, and `year-500.json`. The current long-run probe reached year 500 with 5 civilizations, 105 settlements, 90 roads, 170,832 total people, 27,245 living people, 2,446,665 legend events, 663,339 memories, and about 8.9 GB RSS.
